 ...
 */
export function NarrativeTimeline({ blocks = NARRATIVE, className = "" }: NarrativeTimelineProps) {
...
function BlockItem({ block, inView, slideFrom }: BlockItemProps) {
    return (
        ...
        >
            {block.kind === "prose" ? <ProseBlock block={block} /> : <PhotoBlock block={block} />}
        </motion.div>
    )
}
        ...
        <TiltCard max={6}>
            <GlassCard hover={false} className="p-2">
                <div
                    ...
                    style={{ aspectRatio: block.aspectRatio ?? "4/3" }}
                >
